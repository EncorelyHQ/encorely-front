...
import { useHomeProfile } from '@/modules/home/hooks/useHomeProfile';
...
export default function HomeScreen() {
  ...
  const { swipeCount: swipesCount } = useHomeProfile();
  const hasApiRadarAccess = swipesCount >= API_SWIPE_THRESHOLD;
      ...
      <StyledSafeArea>
        <ContentScroller showsVerticalScrollIndicator={false}>
          ...
          {/* Swipe/Radar Action Card */}
           <TouchableOpacity activeOpacity={0.8} onPress={() => router.push(hasApiRadarAccess ? '/(main)/radar' : '/(main)/')}>
              <GlassCard intensity={40} tint="dark" style={{ borderColor: hasApiRadarAccess ? '#F366FF' : 'rgba(255,255,255,0.1)' }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                  <SpotifyStatusText>{hasApiRadarAccess ? 'Radar Social Activo' : 'Sound-Swipe'}</SpotifyStatusText>
                  <Ionicons name="chevron-forward" size={20} color="#fff" />
                </View>
                <SpotifyStatusSub style={{ marginBottom: 16 }}>
                  {hasApiRadarAccess
                    ? 'Descubre fans en eventos con tu mismo vibe.'
                    : `Evalúa tracks para desbloquear el radar (${API_SWIPE_THRESHOLD} swipes mínimos).`}
                </SpotifyStatusSub>
                ...
                <Text style={{ color: 'rgba(255,255,255,0.5)', fontSize: 11, marginTop: 8, textAlign: 'right' }}>
                  {swipesCount} / {RADAR_SWIPES_THRESHOLD} swipes · API: {API_SWIPE_THRESHOLD}+
                </Text>
              </GlassCard>
           </TouchableOpacity>
